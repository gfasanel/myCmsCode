...
void macro_MakeGsfMu()
{
  ...
  float yRangeMax[6] = {1000, 1000, 1000, 100000, 100000, 100000};
  ...
  for (int k = 0; k < 3; ++k) {
    ...
    for (unsigned int j = 0; j < 2; ++j) {
      ...
      if (k == 2) k = -1;
      // make the histograms
      TH1F *dataOverBgHist = (TH1F *)MakeHistoFromBranch(&input, "frEmuTree_data", "mass", k, eRegion, "", 0., 0., binning, 0x300, normToBin);
      emuMass_data.push_back(MakeHistoFromBranch(&input, "frEmuTree_data", "mass", k, eRegion, "", 0., 0., binning, 0x300));
      TH1F *ttbarComb = MakeHistoFromBranch(&input, "frEmuTree_ttbar", "mass", k, eRegion, "genMTtbar", 0., 700., binning, 0x3DE, normToBin);
      totMcWeight = 1. / (1 / mcWeight->GetVal() + 1 / mcWeight700to1000->GetVal());
      ttbarComb->Add(MakeHistoFromBranch(&input, "frEmuTree_ttbar", "mass", k, eRegion, "genMTtbar", 700., 1000., binning, 0x39E, normToBin), totMcWeight);
      ttbarComb->Add(MakeHistoFromBranch(&input, "frEmuTree_ttbar700to1000", "mass", k, eRegion, "genMTtbar", 700., 1000., binning, 0x39E, normToBin), totMcWeight);
      totMcWeight = 1. / (1 / mcWeight->GetVal() + 1 / mcWeight1000up->GetVal());
      ttbarComb->Add(MakeHistoFromBranch(&input, "frEmuTree_ttbar", "mass", k, eRegion, "genMTtbar", 1000., 1000000000., binning, 0x39E, normToBin), totMcWeight);
      ttbarComb->Add(MakeHistoFromBranch(&input, "frEmuTree_ttbar1000up", "mass", k, eRegion, "genMTtbar", 1000., 1000000000., binning, 0x39E, normToBin), totMcWeight);
      emuMass_ttbar.push_back(ttbarComb);
      //emuMass_ttbar.push_back(MakeHistoFromBranch(&input, "frEmuTree_ttbar", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      //emuMass_ttbar.push_back(MakeHistoFromBranch(&input, "frEmuTree_ttbarto2l", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_ztautau.push_back(MakeHistoFromBranch(&input, "frEmuTree_ztautau", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_ww.push_back(MakeHistoFromBranch(&input, "frEmuTree_ww", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_wz.push_back(MakeHistoFromBranch(&input, "frEmuTree_wz", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_zz.push_back(MakeHistoFromBranch(&input, "frEmuTree_zz", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_tw.push_back(MakeHistoFromBranch(&input, "frEmuTree_tw", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_zmumu.push_back(MakeHistoFromBranch(&input, "frEmuTree_zmumu", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_zee.push_back(MakeHistoFromBranch(&input, "frEmuTree_zee", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      emuMass_wjets.push_back(MakeHistoFromBranch(&input, "frEmuTree_wjets", "mass", k, eRegion, "", 0., 0., binning, 0x3DE, normToBin));
      if (k == -1) k = 2;
      ...
      if (qcdEst > 0) {
        if (k == 2) k = -1;
        //if (qcdEst == 2) qcdContrib = MakeHistoFromBranch(&input, "frEmuTree_data", "mass", ALL, eRegion, "", 0., 0., binning, 0x300);
        if (qcdEst == 2) {
          qcdContrib = MakeHistoFromBranch(&input, "frEmuTree_data", "mass", k, eRegion, "", 0., 0., binning, 0x300);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_ttbar", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_ztautau", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_ww", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_wz", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_zz", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_tw", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_zmumu", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_zee", "mass", k, eRegion, "", 0., 0., binning, 0x3DE), -1.);
          //qcdContrib->Add(MakeHistoFromBranch(&input, "frEmuTree_wjets", "mass", k, eRegion, "", 0., 0., binning, 0x7DE), -1.);
          emuMass_wjets.back()->Scale(0.);
        }
        emuMass_qcd.push_back((TH1F *)qcdContrib->Clone("emuMass_" + histoSign[k] + "qcd"));
        //if (qcdEst == 2 && k != ALL) emuMass_qcd.back()->Scale(0.5);
...
// flags: [apply fake rate | fail HEEP | pass Trigger | lumi | MC weight | trigger Eff | trigger MC to data SF | lumi SF | ele SF | mu SF | PU reweight]
TH1F *
MakeHistoFromBranch(TFile *input, const char *treeName, const char *brName, int signs, int &region, const char *cutVariable, float cutLow, float cutHigh, vector<float> &binning, unsigned int flags, bool normToBinWidth, float userScale)
{
  ...
  for (unsigned int i = 0; i < nEntries; ++i) {
    ...
    if ((flags & 1<<9) && passHeep) continue;
    if (flags & 1<<10) scaleFactor *= fakeRate / (1 - fakeRate);
