...
for brName, xAxisTitle in zip(brNames, xAxisTitles):
    cList.append(TCanvas(brName, brName, 600, 600))
    cList[len(cList)-1].cd()
    
    #get the histograms from the tree
    treeMzMa1000.Draw(brName+'>>hMzMa1000_{0}(30)'.format(brName))
    hMzMa1000 = gROOT.FindObject('hMzMa1000_{0}'.format(brName))
    treeMz1000.Draw(brName+'>>hMz1000_{0}({1},{2},{3})'.format(brName, hMzMa1000.GetNbinsX(), hMzMa1000.GetXaxis().GetXmin(), hMzMa1000.GetXaxis().GetXmax()))
    hMz1000 = gROOT.FindObject('hMz1000_{0}'.format(brName))
    treeMa1000.Draw(brName+'>>hMa1000_{0}({1},{2},{3})'.format(brName, hMzMa1000.GetNbinsX(), hMzMa1000.GetXaxis().GetXmin(), hMzMa1000.GetXaxis().GetXmax()))
    hMa1000 = gROOT.FindObject('hMa1000_{0}'.format(brName))
